...
export default function Quantity(props) {
  ...
  function handleShow(breakpoint) {
    ...
    setShow(true);
      ...
      fetch(props.url+"/moh/" + localStorage.getItem("compname") + "/Stock/Branch/"+ props.oData["ItemNo"].trim()+"/" )
        .then((resp) => resp.json())
        .then((data) => {
    ...
    const DFormat = () =>{
      if (l == "btn"){
        return <Button key={uuid()} className="me-2 my-1" onClick={() => handleShow(true)}>
        Qty: {props.children}
        </Button> 
      }
      else {
        return <td key={uuid()} className="me-2 my-1" onDoubleClick={() => handleShow(true)}>
            {props.children}
            </td>
